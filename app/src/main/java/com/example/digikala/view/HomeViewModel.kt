...
class HomeViewModel(val homeRepositoryImp: HomeRepositoryImp) : ViewModel() {
    ...
    fun getHomeData(token: String) {
        viewModelScope.launch {
            try {
                homeRepositoryImp.getHomeData(token)
                    .onStart { _homeData.value = NetworkState.Loading }
                    .catch { e -> _homeData.value = NetworkState.Failure(e) }
                    .collect { response -> _homeData.value = NetworkState.Success(response) }
            } catch (e: Exception) {
